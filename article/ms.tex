...
\title{Detection of millions of stellar binaries using \Gaia\ data}
...
\begin{itemize}
\item the binary fraction is important
...
\item here we do that for lots of sources
\end{itemize}
...
These methods are sensitive to binary systems with different properties.
...
We caution that this deductive inference is potentially far more uncertain (or
contaminated) than the binary probabilities we calculate from the radial
velocity jitter, astrometric excess noise, and photometric colours. Here we are
explicitly assuming that if a source is bright enough and within the right 
colour range, then a radial velocity would be provided \emph{unless} the source
is a spectroscopic binary. In Section \ref{sec:discussion} we detail some of the
immediate limitations or interpretations of this assumption, but here we simply
note that the probability of a source being a double-lined spectroscopic binary
is calculated (or rather, deduced) separately to our calculations of binarity
probability given \emph{measured} quantities like radial velocity, astrometry,
and photometric colours.

\begin{itemize}
    \item completeness
    \item cross-check against SB catalogues -- does it match expectations?
    \item what about close to the plane? 
    \item when and why is this bad?
\end{itemize}
