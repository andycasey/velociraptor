...
one_to_one_line_kwds = dict(c="#666666", linestyle=":", lw=0.5, zorder=-1)

def savefig(fig, basename):
    prefix = f"battery-stars-{basename}-v{RELEASE_CANDIDATE_VERSION}"
...
figure_name = "rv_tau_sb9_vs_soubiran"
if figure_name in MAKE_FIGURES:
    ...
    tau_soubiran = np.copy(vl_soubiran_subset["rv_tau_single"])
    
    # Umm, we need some correction here.
    print("WARNING ANDY CHECK THIS")
    K_soubiran = vl_soubiran_subset["rv_single_epoch_scatter"] - vl_soubiran_subset["rv_mu_single"]
    tau_soubiran[K_soubiran <= 0] = 1.0

    tau_soubiran = tau_soubiran[np.isfinite(tau_soubiran)]

    tau_sb9 = np.copy(vl_sb9_subset["rv_tau_single"])
    K_sb9 = vl_sb9_subset["rv_single_epoch_scatter"] - vl_sb9_subset["rv_mu_single"]
    
    tau_sb9[K_sb9 <= 0] = 1.0
    tau_sb9 = tau_sb9[np.isfinite(tau_sb9)]

    fig, ax = plt.subplots(1, 1, figsize=(4.5, 4.5))
    ax.hist([tau_soubiran, tau_sb9],
            label=(
                r"\textrm{Single stars (Soubiran et al. 2013)}",
                r"\textrm{Binaries (Pourbaix et al. 2004)}",
            ),
            bins=20, stacked=False)

    ax.set_xlabel(r"\textrm{single star probability} $\tau_\textrm{rv}$")
...
if figure_name in MAKE_FIGURES:
    ...
    #c = np.copy(vl_sb9_subset["rv_tau_single"])
    #c[K_est < 0] = 1.0

    #log_K_significance[vl_sb9_ids]
    ...
    scat = ax.scatter(x, y, c=c, s=10, rasterized=True)
    ...
    cbar.set_major_locator(MaxNLocator(5))


    #cbar.set_label(r"\textrm{single star fraction} $\tau_\textrm{single}$")
    cbar.set_label(r"$\log_{10}(P \textrm{/\,days})$")
    fig.tight_layout()

    #ax.set_xlabel(r"$K/P\sqrt{1 - e^2}$ \textrm{(SB9)}")
    #ax.set_ylabel(r"$K_\textrm{est} /P\sqrt{1 - e^2}$ \textrm{(this work)}")
    ax.set_xlabel(r"$K_{1} / \alpha$ \textrm{(Pourbaix et al. 2014)}")
    ax.set_ylabel(r"$K_{1} / \alpha$ \textrm{(this work)}")
