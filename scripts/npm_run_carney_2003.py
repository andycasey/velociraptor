...
with open("the-battery-stars.rv.yaml", "r") as fp:
    config = yaml.load(fp)
config["results_path"] = "results/carney-2003-singles.rv_single_epoch_scatter.pkl"


with open("the-battery-stars.astrometry.yaml", "r") as fp:
    config = yaml.load(fp)
config["results_path"] = "results/carney-2003-singles.astrometric_unit_weight_error.pkl"
...
t = Table.read("data/carney_2003_singles.fits")
carney_indices = np.where(np.in1d(data["source_id"], t["source_id"]))[0]



sp_swarm(*carney_indices)
...
with open("results/carney-2003-singles.unit_weight_astrometric_error.pkl", "rb") as fp:
    astrometric_results = pickle.load(fp)

with open("results/carney-2003-singles.rv_single_epoch_scatter.pkl", "rb") as fp:
    rv_results = pickle.load(fp)

subset = Table(data[carney_indices])

keys = ("theta", "mu_single", "sigma_single", "mu_multi", "sigma_multi")

for i, key in enumerate(keys):
    subset["rv_{}".format(key)] = rv_results[carney_indices, i]
    subset["astrometric_{}".format(key)] = astrometric_results[carney_indices, i]

finite = np.isfinite(subset["astrometric_theta"] * subset["rv_theta"])
subset = subset[finite]
print(len(subset))

subset.write("results/carney_2003_singles.results.fits", overwrite=True)
